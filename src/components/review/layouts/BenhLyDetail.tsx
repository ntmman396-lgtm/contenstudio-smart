...
import { ImageDropZone, RelationField, RichTextEditor, Toggle, FAQEditor } from '../DetailShared';
...
export default function BenhLyDetail({ article, onSave, onPublish, onBack, onSoftUpdate }: LayoutProps) {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      const previewFields = toStrapiPreview(data.article);
      // Thực hiện trích xuất ngay trên nội dung mới
      const extractedPreviewFields = handleExtractAllSections(true, previewFields.moTa, previewFields) || previewFields;

      setFields(extractedPreviewFields);
      // Lưu thẳng trạng thái mới
      onSave({ ...article, ...data.article, id: article.id }, extractedPreviewFields);
  ...
  const handleExtractAllSections = (force = false, sourceHtml?: string, baseFields?: StrapiArticleFields) => {
    const currentFields = baseFields || fields;
    // Only run if trieuChung is empty (meaning we haven't extracted yet)
    if (!force && currentFields.trieuChung && currentFields.trieuChung.length > 0) return null;

    try {
      const parser = new DOMParser();
      const doc = parser.parseFromString(sourceHtml || currentFields.moTa, 'text/html');
      
      let currentKey: keyof StrapiArticleFields | 'faq' = 'moTa';
      const extractedHtml: Record<string, string> = {
         moTa: '',
         trieuChung: '',
         nguyenNhan: '',
         nguyCo: '',
      ...
      while (sibling) {
        if (sibling.tagName === 'H2') {
           ...
           let matchedSection = '';
           if (headingText.includes('triệu chứng') || headingText.includes('dấu hiệu')) matchedSection = 'trieuChung';
           else if (headingText.includes('nguyên nhân')) matchedSection = 'nguyenNhan';
           else if (headingText.includes('nguy cơ')) matchedSection = 'nguyCo';
           else if (headingText.includes('chẩn đoán') || headingText.includes('điều trị')) matchedSection = 'chanDoanDieuTri';
           else if (headingText.includes('phòng ngừa') || headingText.includes('sinh hoạt')) matchedSection = 'sinhHoatPhongNgua';
           else if (headingText.includes('câu hỏi') || headingText.includes('faq')) matchedSection = 'faq';
           else if (headingText.includes('tìm hiểu chung') || headingText.includes('bệnh học') || headingText.includes('tổng quan')) matchedSection = 'moTa';

           if (matchedSection) {
              currentKey = matchedSection as any;
              // Do not append the H2 tag itself into the section, skip it
        ...
        if (currentKey === 'faq') {
          ...
          if (sibling.tagName === 'H3') {
             ...
             currentQuestion = (sibling.textContent?.trim() || '').replace(/^\d+[\.\)\-]\s*/, '');
             currentAnswer = '';
          } else if (sibling.tagName === 'P') {
             const strongNode = sibling.querySelector('strong');
             
             // Define a heuristic for whether this P tag represents a new question
             // Either it has a strong tag containing a question mark, or its entire text looks like a question
             const isNewQuestion = strongNode && (strongNode.textContent?.includes('?') || sibling.textContent?.includes('Câu'));
             ...
             if (isNewQuestion) {
                ...
                currentQuestion = (strongNode!.textContent?.trim() || sibling.textContent?.split('?')[0] + '?' || '').replace(/^\d+[\.\)\-]\s*/, '');
                
                // The answer is the rest of the P tag
                const clonedNode = sibling.cloneNode(true) as HTMLElement;
                const clonedStrong = clonedNode.querySelector('strong');
                if (clonedStrong) clonedStrong.remove();
                
                currentAnswer = clonedNode.textContent?.trim() || '';
                if (currentAnswer.startsWith(':') || currentAnswer.startsWith('-')) {
                   currentAnswer = currentAnswer.substring(1).trim();
                }
             } else {
                currentAnswer += (sibling.textContent || '').trim() + '\n\n';
             }
          } else if (sibling.tagName === 'UL' || sibling.tagName === 'OL') {
             const lis = Array.from(sibling.querySelectorAll('li'));
             currentAnswer += lis.map(li => '- ' + li.textContent).join('\n') + '\n\n';
          }
        } else {
          // Normal section appending
          extractedHtml[currentKey] += sibling.outerHTML;
        }
      ...
      newFields.moTa = extractedHtml.moTa || currentFields.moTa;
      newFields.trieuChung = extractedHtml.trieuChung || currentFields.trieuChung;
      newFields.nguyenNhan = extractedHtml.nguyenNhan || currentFields.nguyenNhan;
      newFields.nguyCo = extractedHtml.nguyCo || currentFields.nguyCo;
      newFields.chanDoanDieuTri = extractedHtml.chanDoanDieuTri || currentFields.chanDoanDieuTri;
      newFields.sinhHoatPhongNgua = extractedHtml.sinhHoatPhongNgua || currentFields.sinhHoatPhongNgua;
      if (faqs.length > 0 && (!currentFields.faq || currentFields.faq.length === 0)) {
        newFields.faq = faqs;
      }
      
      if (baseFields) return newFields; // Return if called externally with baseFields

      setFields(newFields);
      setEditorKey(k => k + 1); // remount all editors with clean HTML
  ...
  React.useEffect(() => {
    // If FAQ is missing but raw HTML contains 'faq' or 'câu hỏi', force extraction
    const hasFaqInContent = fields.moTa.toLowerCase().includes('faq') || fields.moTa.toLowerCase().includes('câu hỏi');
    if ((!fields.faq || fields.faq.length === 0) && hasFaqInContent) {
      handleExtractAllSections(true);
    } else {
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-xl font-bold text-[var(--text-primary)]">Bệnh Lý</h1>
          ...
          <p className="text-xs text-[var(--text-muted)]">API ID : disease-article &nbsp;·&nbsp; Template: Bệnh Lý</p>
        </div>
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 space-y-5 min-w-0">
            ...
            <div className="glass-card p-5 rounded-xl border border-[var(--lc-primary)]/20 relative overflow-hidden">
              ...
              <RichTextEditor
                key={`editor-mt-${editorKey}`}
                label="Tìm Hiểu Chung"
                value={fields.moTa}
            ...
            {/* ── Triệu Chứng ── */}
            <div className="glass-card p-5 rounded-xl border border-gray-500/20">
              <RichTextEditor
                key={`editor-tc-${editorKey}`}
                label="Triệu Chứng"
                value={fields.trieuChung || ''}
                onChange={(v) => updateField('trieuChung', v)}
                minHeight={200}
              />
            </div>
            ...
            <div className="glass-card p-5 rounded-xl border border-gray-500/20">
              <RichTextEditor
                key={`editor-nn-${editorKey}`}
                label="Nguyên Nhân"
                value={fields.nguyenNhan || ''}
                onChange={(v) => updateField('nguyenNhan', v)}
                minHeight={200}
              />
            </div>
            ...
            <div className="glass-card p-5 rounded-xl border border-gray-500/20">
              <RichTextEditor
                key={`editor-nc-${editorKey}`}
                label="Nguy Cơ"
                value={fields.nguyCo || ''}
                onChange={(v) => updateField('nguyCo', v)}
                minHeight={200}
              />
            </div>

            {/* ── Chẩn Đoán & Điều Trị ── */}
            <div className="glass-card p-5 rounded-xl border border-[var(--lc-primary)]/10">
              <RichTextEditor
                key={`editor-cd-${editorKey}`}
                label="Chẩn Đoán & Điều Trị"
                value={fields.chanDoanDieuTri || ''}
                onChange={(v) => updateField('chanDoanDieuTri', v)}
                minHeight={250}
              />
            </div>
            ...
            <div className="glass-card p-5 rounded-xl border border-emerald-500/20">
              <RichTextEditor
                key={`editor-sh-${editorKey}`}
                label="Sinh Hoạt & Phòng Ngừa"
                value={fields.sinhHoatPhongNgua || ''}
                onChange={(v) => updateField('sinhHoatPhongNgua', v)}
                minHeight={200}
              />
            </div>
            ...
            <div className="glass-card p-5 rounded-xl border border-blue-500/20">
               <FAQEditor 
                 ...
                 onAutoExtract={() => handleExtractAllSections(true)}
               />
            </div>
            ...
            <div className="glass-card p-4 rounded-xl">
              ...
              <div className="space-y-2 text-[11px]">
                <div className="flex justify-between">
                  ...
                  <span className="text-[var(--text-secondary)] font-medium">Bệnh Lý</span>
                </div>
                <div className="flex justify-between">

...
export default function HoiDapBacSiDetail({ article, onSave, onPublish, onBack, onSoftUpdate }: LayoutProps) {
  ...
  const handleExtractFAQ = () => {
    try {
      const parser = new DOMParser();
      const doc = parser.parseFromString(fields.moTa, 'text/html');
      
      const faqs: { question: string, answer: string }[] = [];
      let currentQuestion = '';
      let currentAnswer = '';
      
      // Find the H2 that contains FAQ keywords
      const h2s = Array.from(doc.querySelectorAll('h2'));
      const faqH2 = h2s.find(h => h.textContent?.toLowerCase().includes('câu hỏi liên quan') || h.textContent?.toLowerCase().includes('faq'));
      
      if (!faqH2) {
        setToastMessage('Không tìm thấy khối chứa H2 "Các câu hỏi liên quan"!');
        return;
      }

      const nodesToRemove: Element[] = [faqH2];
      let sibling = faqH2.nextElementSibling;
      
      while (sibling) {
        if (sibling.tagName === 'H2') break; // Next major section
      ...
      if (faqs.length > 0) {
         // remove nodes from original HTML so they don't appear in RichText anymore
         nodesToRemove.forEach(n => n.remove());
         
         updateField('faq', faqs);
         updateField('moTa', doc.body.innerHTML);
         setEditorKey(k => k + 1); // Force RichTextEditor to reload with cleaned HTML
         // Toast is silent on auto-extract to avoid spam, or we can just say "Auto extracted FAQ"
         // setToastMessage(`✨ Đã tự động bóc tách ${faqs.length} câu FAQ!`);
      }
    } catch(e) { console.error(e); }
  };

  // Auto-extract FAQ when the component mounts
  React.useEffect(() => {
    if ((!fields.faq || fields.faq.length === 0) && fields.moTa.toLowerCase().includes('câu hỏi liên quan')) {
      handleExtractFAQ();
    }
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-xl font-bold text-[var(--text-primary)]">Hỏi Đáp Bác Sĩ</h1>
          ...
          <p className="text-xs text-[var(--text-muted)]">API ID : health-article &nbsp;·&nbsp; Template: Hỏi Đáp Bác Sĩ</p>
        </div>
            ...
            <div className="glass-card p-4 rounded-xl">
              ...
              <div className="space-y-2 text-[11px]">
                <div className="flex justify-between">
                  ...
                  <span className="text-[var(--text-secondary)] font-medium">Hỏi Đáp Bác Sĩ</span>
                </div>
                <div className="flex justify-between">

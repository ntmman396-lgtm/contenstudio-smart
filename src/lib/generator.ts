...
function buildSingleUserPrompt(
  ...
  const isNarrative = settings.templateId === 'controlled-narrative' || settings.templateId === 'storytelling';
  ...
  const outlineText = item.outline 
    ...
    : (isQA 
        ...
        : (isNarrative
            ? `Dàn ý: (Chưa có — người dùng không cung cấp) TỰ ĐỘNG XÂY DỰNG DÀN Ý THEO FORMAT CONTROLLED NARRATIVE + DOCTOR ANCHOR CHUẨN (Opening -> H2 Misunderstanding -> H2 Mechanism/Evidence -> H2 Decision -> H2 Safety -> Ending Callback).`
...
2. Trường "content" PHẢI chứa toàn bộ nội dung HTML bài viết hoàn chỉnh (dùng <h2>, <h3>, <p>, <ul>, <li>, <strong>, <em>, <table>, <blockquote>). NỘI DUNG PHẢI ĐẦY ĐỦ, CHI TIẾT, KHÔNG TÓM TẮT.
...
10. QUY TẮC PHÂN CẤP HEADING (H2 → H3 — BẮT BUỘC ≥ 2 H3s):
   - Nếu một mục H2 có chứa H3 thì BẮT BUỘC phải có từ 2 thẻ H3 trở lên (≥ 2 H3s) để phân chia ít nhất 2 khía cạnh, 2 nhóm đối tượng hoặc 2 bước thực hiện rõ ràng.
